...
void MatrixMultiply::FinishHostExecution(bool freeHostMemory)
{
  ...
  // Update timers
  ERROR_CHECK(cudaSetDevice(m_deviceNum)); // May not be necessary?
  ...
  std::chrono::duration< double > diff(m_streamStarted - m_queueStarted);
  m_queueTimeMS = (float)(1000.0 * diff.count());
